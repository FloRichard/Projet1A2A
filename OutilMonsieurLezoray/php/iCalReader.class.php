...
class ICal
{
     ...
     */ 
    public function __construct($filename,$cxContext="") 
    {
        if (!$filename) {
            return false;
        }
        if(empty($cxContext)) $lines = file($filename, FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES);
        else $lines = file($filename, FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES,$cxContext);
        $possibleCodes=array("BEGIN","END","METHOD","PRODID","VERSION","CALSCALE","DTSTAMP","DTSTART","DTEND",
                "SUMMARY","LOCATION","DESCRIPTION","UID","CREATED","LAST-MODIFIED","SEQUENCE",);
        if (stristr($lines[0], 'BEGIN:VCALENDAR') === false) {
            ...
            foreach ($lines as $line) {
                ...
                $codeOK=array_search($keyword,$possibleCodes);
                if(!is_integer($codeOK) && $codeOK==false) {
                    $this->cal[$type][$this->event_count - 1]["$previousKeyword"].= "$keyword $value"; 
                    //multiligne=>il faut concaténer $value avec le code précédent
                }
                else {
